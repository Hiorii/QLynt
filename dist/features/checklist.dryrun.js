(() => {
    ...
    const generateChecklistDryRun = ({ title, description, hasAC }) => {
        ...
        const warnings = [];
        if (!description)
            warnings.push("Uzupełnij opis (bez tego testy to wróżenie z fusów)");
        if (!hasAC)
            warnings.push("Dodaj Acceptance Criteria (min. 2–3 punkty)");
        const hints = inferDomainHints(`${title}\n${description}`);
        const domain = [];
        if (hints.includes("auth"))
            domain.push("Auth: poprawne przekierowania po zalogowaniu/wylogowaniu", "Auth: błędne hasło / brak dostępu → poprawny komunikat", "Auth: odświeżenie strony (F5) nie psuje sesji");
        if (hints.includes("payments"))
            domain.push("Payments: poprawne kwoty, waluty, rounding", "Payments: double submit / retry nie robi duplikatów", "Payments: statusy transakcji poprawnie się aktualizują");
        if (hints.includes("permissions"))
            domain.push("Permissions: user bez uprawnień → blokada + jasny komunikat", "Permissions: role edge-case (np. viewer/editor) zachowują się poprawnie");
        if (hints.includes("upload"))
            domain.push("Upload: limity rozmiaru i typy plików", "Upload: przerwanie uploadu / ponowienie", "Upload: nazwy plików ze znakami specjalnymi");
        if (hints.includes("search"))
            domain.push("Search: kombinacje filtrów, sort + paginacja", "Search: brak wyników → poprawny empty state", "Search: wydajność na większej liście");
        if (hints.includes("api"))
            domain.push("API: obsługa 4xx/5xx + timeouty", "API: kontrakt response (schema) zgodny z oczekiwaniami", "API: retry/backoff (jeśli jest) działa poprawnie");
        if (hints.includes("ui"))
            domain.push("UI: overflow na długich tekstach", "UI: focus/Tab (podstawowa a11y)", "UI: ciemny motyw (jeśli jest) / kontrasty");
        const out = [
            ...warnings.map((w) => `⚠️ ${w}`),
            ...base,
            ...domain
